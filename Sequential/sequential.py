...
class SequentialFile:
    ...
    def _load_all(self):
        ...
        with open(self.filename, 'rb') as file:
            ...
            for i in range(count):
                data = file.read(Registro.SIZE + 1)
                if not data or len(data) < (Registro.SIZE + 1):
                    break
                reg = Registro.from_bytes(data[:Registro.SIZE])
                deleted = struct.unpack('B', data[Registro.SIZE:])[0]
                if deleted == 0:
        ...
        with open(self.aux_filename, 'rb') as aux_file:
            while True:
                data = aux_file.read(Registro.SIZE)
    ...
    def _binary_search(self, key):
        ...
        with open(self.filename, 'r+b') as file:
            while left <= right:
                mid = (left + right) // 2
                offset = self.HEADER_SIZE + mid * (Registro.SIZE + 1)
                file.seek(offset)
                data = file.read(Registro.SIZE + 1)
                if len(data) < Registro.SIZE + 1:
                    break
    ...
    def search(self, key):
        ...
        if pos is not None:
            with open(self.filename, 'rb') as file:
                ...
                while i < count: 
                    offset = self.HEADER_SIZE + i * (Registro.SIZE + 1)
                    file.seek(offset)
                    data = file.read(Registro.SIZE + 1)
                    if len(data) < Registro.SIZE + 1:
                        break
        ...
        with open(self.filename, 'r+b') as file:
            ...
            while i < count: # Buscamos mas de uno
                ...
                if reg_key == key: # Borramos
                    if deleted == 0:
                        file.seek(offset + Registro.SIZE)
                        file.write(b'\x01')
                        count -= 1
                        deleted_any = True
                    i += 1
